...
//BLOCKSIZE: The total transfer is broken up into blocks of this size.
//           This value should be multiple of the cache line size and should
//           not exceed the total number of bytes in the transfer. 

#define BUFSIZE     (32*CACHE_L2_LINESIZE)
#define DATASIZE    (4*BUFSIZE)
#define BLOCKSIZE   (2*CACHE_L2_LINESIZE)
...
void main()
{
    ...
    CACHE_wbInvL2_blocks(src_data_ext, DATASIZE, BLOCKSIZE);  
    CACHE_wbInvL2_blocks(dst_data_ext, DATASIZE, BLOCKSIZE);  
...
void copy_and_process()
{
    ...
    for (i=0; i<(DATASIZE/BUFSIZE)-2; i+=2)
    {
        ...
        //  L2 Cache.
        CACHE_wbL2_blocks(OutBuffA, BUFSIZE, BLOCKSIZE);  
        ...
        CACHE_wbL2_blocks(OutBuffB, BUFSIZE,BLOCKSIZE);  
